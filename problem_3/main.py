...
left_pts = np.float32([ kp1[m[0].queryIdx].pt for m in matches ]).reshape(-1,1,2)
right_pts = np.float32([ kp2[m[0].trainIdx].pt for m in matches ]).reshape(-1,1,2)
r2l_H, _ = cv2.findHomography(left_pts, right_pts, cv2.RANSAC, 5.0)
...
stitched_image = cv2.warpPerspective(r_img, r2l_H, (l_img.shape[1] + r_img.shape[1], l_img.shape[0]))
